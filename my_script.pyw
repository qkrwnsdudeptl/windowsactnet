...
class SystemUtilityApp:
    def __init__(self, master):
        self.master = master
        master.title("시스템 유틸리티 (로그 강화 최종본)")
        ...
        log_frame = ttk.LabelFrame(master, text=" 실행 로그 (1번 버튼 문제 발생 시 이 내용을 알려주세요) ")
    ...
    # === 이 함수에 상세 로그가 추가되었습니다 ===
    def get_netsh_compatible_name(self):
        self.log("[네트워크] 'netsh용 이름' 검색 시작...")
        try:
            self.log("[네트워크] 1/3: 'route print -4 0.0.0.0' 실행 시작...")
            route_output = self.run_command("route print -4 0.0.0.0");
            self.log("[네트워크] 1/3: 'route print -4 0.0.0.0' 실행 완료.")
            if not route_output: self.log("[네트워크] 오류: route_output이 비어있습니다."); return None
            
            match = re.search(r"^\s*0\.0\.0\.0\s+0\.0\.0\.0\s+[\d\.]+\s+([\d\.]+)\s+.*$", route_output, re.MULTILINE)
            if not match: self.log("[네트워크] 오류: route_output에서 기본 경로 IP를 찾지 못했습니다."); return None
            interface_ip = match.group(1)
            self.log(f"[네트워크] 활성 IP 발견: {interface_ip}")

            self.log("[네트워크] 2/3: 'ipconfig /all' 실행 시작...")
            ipconfig_output = self.run_command("ipconfig /all")
            self.log("[네트워크] 2/3: 'ipconfig /all' 실행 완료.")
            if not ipconfig_output: self.log("[네트워크] 오류: ipconfig_output이 비어있습니다."); return None
            
            # Link-local IPv6 주소에서 인덱스 추출
            ipv6_match = re.search(r"링크-로컬 IPv6 주소[ .]+: .+(%\d+)", ipconfig_output)
            if not ipv6_match: self.log("[네트워크] 오류: ipconfig_output에서 링크-로컬 IPv6 주소 패턴을 찾지 못했습니다."); return None
            if_index = ipv6_match.group(1).replace('%','')
            self.log(f"[네트워크] 인터페이스 인덱스 발견: {if_index}")

            self.log("[네트워크] 3/3: 'netsh interface ipv4 show interfaces' 실행 시작...")
            netsh_output = self.run_command("netsh interface ipv4 show interfaces")
            self.log("[네트워크] 3/3: 'netsh interface ipv4 show interfaces' 실행 완료.")
            if not netsh_output: self.log("[네트워크] 오류: netsh_output이 비어있습니다."); return None
            
            self.log("[네트워크] netsh_output 분석 시작...")
            for line in netsh_output.splitlines():
                if line.strip().startswith(if_index):
                    ...
                    if len(parts) > 3:
                        ...
                        return netsh_name
            self.log("[네트워크] 오류: netsh_output에서 해당 인덱스를 가진 인터페이스 이름을 찾지 못했습니다."); return None
        except Exception as e:
            self.log(f"[네트워크] 'netsh용 이름' 검색 중 예외 발생: {e}")
            return None
    ...
    def backup_current_settings(self):
        ...
        subnet_match = re.search(r"(?:서브넷 마스크|Subnet Mask)[\s.]*:\s*([0-9.]+)", output, re.IGNORECASE) # 이 부분은 이전 최종본에서 수정됨
        ...
        settings['subnet'] = subnet_match.group(1).strip() if subnet_match else None # 이 부분은 이전 최종본에서 수정됨
